...
// ================= Header Component =================
const Header = ({ onMenuPress, isLandscape }) => {
  const laserAnim = useRef(new Animated.Value(-200)).current;
...
// ================= Menu Component =================
const HeaderMenu = ({ visible, onClose, onShare }) => {
  const openLink = url => {
    onClose();
    Linking.openURL(url);
  };
...
// ================= GameCard Component =================
const GameCard = React.memo(({ item, onPress, index, isLandscape }) => {
  const baseScale = useRef(new Animated.Value(1)).current;
  const pressScale = useRef(new Animated.Value(1)).current;
  const translateY = useRef(new Animated.Value(30)).current;
  const opacity = useRef(new Animated.Value(0)).current;
  ...
  // Card dimensions
  const cardWidth = isLandscape ? (width - 60) / 2 : width - 24;
  ...
  return (
    <Animated.View
      style={[
        ...
        isLandscape && { marginHorizontal: 10 },
      ]}
    >
      <AnimatedLinearGradient
        colors={getGlowColors(item.id)}
...
export default function HomeScreen() {
  ...
  // Create data for auto-scroll
  const infiniteData = useMemo(() => {
    return [...GAMES, ...GAMES, ...GAMES];
  }, []);

  // Auto-scroll for landscape mode
  useEffect(() => {
    if (!isLandscape || isPaused || !flatListRef.current) return;

    const scroll = () => {
      if (!flatListRef.current || isPaused) return;

      scrollOffset.current += scrollSpeed.current;
      ...
      // Calculate for infinite loop
      const cardWidth = (width - 60) / 2 + 20;
      const totalContentWidth = infiniteData.length * cardWidth;
      const screenWidth = width;

      if (scrollOffset.current >= totalContentWidth - screenWidth) {
  ...
  }, [isLandscape, isPaused, width, infiniteData.length]);

  // Handle touch to pause auto-scroll
  const handleTouchStart = () => {
    if (isLandscape && !isPaused) {
      setIsPaused(true);
  ...
  return (
    <SafeAreaView style={styles.container} onTouchStart={handleTouchStart}>
      ...
      {/* Game List - DIFFERENT LAYOUTS */}
      {isLandscape ? (
        // LANDSCAPE: Horizontal auto-scrolling with 2 cards
        ...
        // PORTRAIT: Vertical list (original layout)
        <FlatList
          data={GAMES}
          keyExtractor={item => item.id}
          renderItem={renderItem}
